...
class DeepSpatialAttention(nn.Module):
    ...
    def forward(self, E, D):
        ...
        out_ = D * self.sigmoid(attn_mask)
        return out_*0.4+out__*0.6
class ShallowSpatialAttention(nn.Module):
    """空间注意力分支"""
    def __init__(self, in_channel, stride=1,ratio=4):
        super(ShallowSpatialAttention, self).__init__()
        inter_channel = in_channel // 2
        self.inter_channel=inter_channel
        self.conv_3 = nn.Sequential(
            nn.Conv2d(in_channel, inter_channel, 1, stride, 0, bias=False),
...
class CSAF(nn.Module):
    ...
    def __init__(self, in_channel, stride=1,i=0):
        super(CSAF, self).__init__()
        self.channel_att = ChannelAttention(in_channel, stride)
        self.shallow_spatial_att = ShallowSpatialAttention(in_channel, stride)
        self.deep_spatial_att = DeepSpatialAttention(in_channel, stride)
        self.isDeep=False
        if i>=2:
            self.isDeep=True
    ...
    def forward(self, E, D):
        ...
        if self.isDeep:
          spa=self.deep_spatial_att(E, D)
        else:
           spa=self.shallow_spatial_att(E, D)
...
class AAA_HWD_U2Net(nn.Module):
    def __init__(self, cfg: dict, out_ch: int = 1):
        ...
        for i,c in enumerate(cfg["decode"]):
            ...
            csaf_list.append(CSAF(int(c[1] / 2),i=i))
            if c[5] is True:
                side_list.append(nn.Conv2d(c[3], out_ch, kernel_size=3, padding=1))
    ...
    def forward(self, x: torch.Tensor) -> Union[torch.Tensor, List[torch.Tensor]]:
        ...
        if self.training:
            ...
            return self.bn(x)
def AAA_HWD_U2Net_V2(out_ch: int = 1):
    cfg = {

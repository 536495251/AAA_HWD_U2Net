...
class DiceMetric:
    ...
    def update(self, preds, labels):
        """
        preds: torch.Tensor, shape [B, 1, H, W], logits or probs
        labels: torch.Tensor, shape [B, 1, H, W], binary masks
        """
        ...
        # 计算 Dice 分数（逐 batch 累积）
        intersection = (preds * labels).sum(dim=1)
        dice = (2. * intersection) / (preds.sum(dim=1) + labels.sum(dim=1) + 1e-8)
...
class PD_FA:
    ...
    def update(self, preds, labels):
        ...
        self.image_area_total = [region.area for region in coord_image]
        self.image_area_match = []
        self.distance_match = []
    ...
    def get(self, img_num):
        Final_FA = self.FA / ((self.img_size * self.img_size) * img_num) if img_num > 0 else 0
        if self.target > 0:
